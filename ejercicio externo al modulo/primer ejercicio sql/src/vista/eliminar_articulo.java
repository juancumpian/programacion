...
public class eliminar_articulo extends JDialog {
			...
			getContentPane().add(buttonPane, BorderLayout.SOUTH);
			{
				...
				okButton.addActionListener(new ActionListener() {
					public void actionPerformed(ActionEvent e) {
						...
						try {
							ID = Principal.s.executeQuery("select ID from articulos where ID="+Integer.parseInt(textid.getText().toString()));
							if(ID.next()){
								JOptionPane.showMessageDialog(null, "borrado");
								operaciones.eliminarID(Integer.parseInt(textid.getText().toString()));
							}else
							...
							e1.printStackTrace();
								...
								ResultSet nombre;
								try {
									nombre = Principal.s.executeQuery("select nombre from articulos where nombre='"+textnom.getText()+"'");
									if(nombre.next()){
										JOptionPane.showMessageDialog(null, "borrado");
										operaciones.eliminarnombre(textnom.getText().toString());
									}else
										JOptionPane.showMessageDialog(null, "no existe");
								} catch (NumberFormatException | SQLException e1) {
									e1.printStackTrace();
